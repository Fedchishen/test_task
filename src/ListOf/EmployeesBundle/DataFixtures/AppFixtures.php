...
class AppFixtures extends Fixture {
    ...
    public function load(ObjectManager $manager) {
        // 50тыс(как по заданию) - это очень тяжелая БД для моего ноута.
        //но при желании можно просто поменять $count_em 
        //на то кол-во работников, которое нужно.
        $count_em = 500;
        for ($i = 1; $i <= $count_em; $i++) {
            ...
            if ($position_id == 1) {
                $chief_id = 1;
            }//царь сам себе начальник
            elseif ($position_id == 2) {
                $chief_id = 1;
            }//бояре подчиняются одному царю
            ...
            elseif ($position_id == 5) {
                $chief_id = rand(62, 121);
            }//холопы старостам

            $employees->setChiefId($chief_id); //запишем начальника

            $employees->setPhoto('empty.jpg'); //photo
